...
export default function DistributorClientView({
    ...
    const renderContact = () => (
        <div className="w-full animation-fade-in pt-36 lg:pt-48 pb-20 px-6 flex flex-col items-center">
            ...
            <div className="w-full max-w-[1100px] grid grid-cols-1 lg:grid-cols-[400px_1fr] gap-0 rounded-[28px] overflow-hidden shadow-2xl relative">
                ...
                <div className="bg-gradient-to-br from-[#1b509d] to-[#041d40] p-8 md:p-14 flex flex-col justify-center border border-[#0d2a58]">
                    <div className="space-y-0">
                        {contactData?.Address && (
                            <div className="flex items-start gap-4 md:gap-5 border-b border-light pb-6 md:pb-8 mb-6 md:mb-8" style={{ borderBottomColor: "rgba(255,255,255,0.15)" }}>
                                <div className="bg-[#4aa5f8] p-2.5 md:p-3 rounded-full text-white shrink-0 mt-1 shadow-sm">
                                    <Map className="w-5 h-5 md:w-6 md:h-6" strokeWidth={1.5} />
                                </div>
                                <div className="mt-1">
                                    <h4 className="text-white font-normal text-lg md:text-[22px] mb-1 md:mb-2 tracking-wide">Address</h4>
                                    <p className="text-white/90 text-sm md:text-[15px] leading-relaxed max-w-[200px]">{contactData.Address}</p>
                                </div>
                            </div>
                        )}
                        {contactData?.Phone && (
                            <div className="flex items-start gap-4 md:gap-5 border-b border-light pb-6 md:pb-8 mb-6 md:mb-8" style={{ borderBottomColor: "rgba(255,255,255,0.15)" }}>
                                <div className="bg-white p-2.5 md:p-3 rounded-full text-black shrink-0 mt-1 shadow-sm">
                                    <Phone className="w-5 h-5 md:w-6 md:h-6" strokeWidth={1.5} />
                                </div>
                                <div className="mt-1">
                                    <h4 className="text-white font-normal text-lg md:text-[22px] mb-1 md:mb-2 tracking-wide">Phone</h4>
                                    <p className="text-white/90 text-sm md:text-[15px]">{contactData.Phone}</p>
                                </div>
                            </div>
                        )}
                        {contactData?.Email && (
                            <div className="flex items-start gap-4 md:gap-5 border-b border-light pb-6 md:pb-8 mb-6 md:mb-8" style={{ borderBottomColor: "rgba(255,255,255,0.15)" }}>
                                ...
                                <div className="mt-1">
                                    <h4 className="text-white font-normal text-lg md:text-[22px] mb-1 md:mb-2 tracking-wide">Email</h4>
                                    <p className="text-white/90 text-sm md:text-[15px]">{contactData.Email}</p>
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    {contactData?.FollowUsOn && (
                        <div className="mt-2 md:mt-4">
                            <h4 className="text-white font-normal text-[18px] md:text-[20px] mb-4 md:mb-6 tracking-wide">Follow Us On :</h4>
                            <div className="flex items-center gap-2.5 md:gap-3">
                                {contactData.FollowUsOn.Facebook && (
                                    <a href={contactData.FollowUsOn.Facebook} target="_blank" rel="noreferrer" className="bg-[#4aa5f8] p-2 flex items-center justify-center rounded-full text-white hover:opacity-80 transition-opacity" style={{ width: '34px', height: '34px' }}>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M14.53 22.92v-8.96h2.98l.45-3.48h-3.43V8.25c0-1.01.28-1.7 1.73-1.7h1.85V3.44c-.32-.04-1.42-.14-2.7-.14-2.67 0-4.5 1.63-4.5 4.63v2.55H7.94v3.48h2.97v8.96h3.62z" /></svg>
                                    </a>
                                )}
                                {contactData.FollowUsOn.Twitter && (
                                    <a href={contactData.FollowUsOn.Twitter} target="_blank" rel="noreferrer" className="bg-white p-2 flex items-center justify-center rounded-full text-[#111] hover:opacity-80 transition-opacity" style={{ width: '34px', height: '34px' }}>
                                ...
                                {contactData.FollowUsOn.Youtube && (
                                    <a href={contactData.FollowUsOn.Youtube} target="_blank" rel="noreferrer" className="bg-white p-2 flex items-center justify-center rounded-full text-[#111] hover:opacity-80 transition-opacity" style={{ width: '34px', height: '34px' }}>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M21.582 6.186a2.636 2.636 0 0 0-1.856-1.868C18.087 3.864 12 3.864 12 3.864s-6.087 0-7.726.454A2.635 2.635 0 0 0 2.418 6.186C1.964 7.838 1.964 12 1.964 12s0 4.162.454 5.814a2.636 2.636 0 0 0 1.856 1.868c1.639.454 7.726.454 7.726.454s6.087 0 7.726-.454a2.635 2.635 0 0 0 1.856-1.868c.454-1.652.454-5.814.454-5.814s0-4.162-.454-5.814zM9.953 15.421V8.579L15.93 12l-5.977 3.421z" /></svg>
                                    </a>
                                )}
                                {contactData.FollowUsOn.Instagram && (
                                    <a href={contactData.FollowUsOn.Instagram} target="_blank" rel="noreferrer" className="bg-white p-2 flex items-center justify-center rounded-full text-[#111] hover:opacity-80 transition-opacity" style={{ width: '34px', height: '34px' }}>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"></rect><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"></line></svg>
                                    </a>
                                )}
                                {contactData.FollowUsOn.Printest && (
                                    <a href={contactData.FollowUsOn.Printest} target="_blank" rel="noreferrer" className="bg-white p-2 flex items-center justify-center rounded-full text-[#111] hover:opacity-80 transition-opacity" style={{ width: '34px', height: '34px' }}>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="currentColor" stroke="none"><path d="M12.017 0C5.396 0 .029 5.367.029 11.987c0 5.079 3.158 9.417 7.618 11.162-.105-.949-.199-2.403.041-3.439.219-.937 1.406-5.957 1.406-5.957s-.359-.72-.359-1.781c0-1.663.967-2.911 2.168-2.911 1.024 0 1.518.769 1.518 1.688 0 1.029-.653 2.567-.992 3.992-.285 1.193.6 2.165 1.775 2.165 2.128 0 3.768-2.245 3.768-5.487 0-2.861-2.063-4.869-5.008-4.869-3.41 0-5.409 2.562-5.409 5.199 0 1.033.394 2.143.889 2.741.099.12.112.225.085.345-.09.375-.293 1.199-.334 1.363-.053.225-.172.271-.401.165-1.495-.69-2.433-2.878-2.433-4.646 0-3.776 2.748-7.252 7.951-7.252 4.158 0 7.392 2.967 7.392 6.923 0 4.135-2.607 7.462-6.233 7.462-1.214 0-2.354-.629-2.758-1.379l-.749 2.848c-.269 1.045-1.004 2.352-1.498 3.146 1.123.345 2.306.535 3.55.535 6.607 0 11.985-5.365 11.985-11.987C23.97 5.366 18.624.002 12.017.002z" /></svg>
                                    </a>
                                )}
                            </div>
                        </div>
                ...
                <div className="bg-[#0b0f19] p-8 md:p-12 border border-l-0 border-[#0b0f19]">
