...
export async function POST(request: Request) {
    try {
        ...
        shopsPendingPhotos.forEach(s => {
            const nameStr = s.outletDetails?.shopName || "N/A";
            const areaStr = s.outletDetails?.area || "N/A";
            const mobileStr = s.outletDetails?.mobileNumber || "N/A";
            const emailStr = s.outletDetails?.email || "N/A";
            ...
            csvContent += `${s._id},${name},${area},${s.outletDetails?.pincode || "N/A"},${mobile},${email},${asm},${se}\n`;
        });
        ...
        shopsWithRecentPhotos.forEach(s => {
            ...
            csvContentSuccess += `${s._id},${name},${area},${s.outletDetails?.pincode || "N/A"},${mobile},${email},${asm},${se},"${imageUrls}"\n`;
        });
        ...
        if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
            ...
            if (shopsPendingPhotos.length > 0) {
                shopsPendingPhotos.forEach((s, idx) => {
                    pendingDetails += `${idx + 1}. ${s.outletDetails?.shopName || 'N/A'} (Area: ${s.outletDetails?.area || 'N/A'}, ASM: ${s.asm || 'N/A'})\n`;
                });
            } else {
            ...
            if (shopsWithRecentPhotos.length > 0) {
                shopsWithRecentPhotos.forEach((s, idx) => {
                    successDetails += `${idx + 1}. ${s.outletDetails?.shopName || 'N/A'} (Area: ${s.outletDetails?.area || 'N/A'}, ASM: ${s.asm || 'N/A'})\n`;
                });
            } else {
